...
rechunk $image_name=image_name:
    ...
    podman run --rm "--mount=type=image,src=${image_name}-bootc,target=/chunkah" -e CHUNKAH_CONFIG_STR quay.io/coreos/chunkah build --label ostree.bootable=1 --compressed --max-layers 128 | \
        podman load | \
        sort -n | \
        head -n1 | \
        cut -d, -f2 | \
